function deepLink(options) {
  ...
  if (path.includes('url=exploreapp')) {
    fallbackPath = decodeURIComponent(path).replace('url=exploreapp://', '');
  }
  ...
  function androidLaunch() {
    if (ua.match(/Chrome/)) {
      ...
      window.onload = function() {
        var myDiv = document.getElementById('androidButton');
        var createA = document.createElement('a');
        var createAText = document.createTextNode('Continue to ExploreTO');
        createA.setAttribute('href', urls.android_intent);
